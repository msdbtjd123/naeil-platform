...
def network_info(instance_id):
	try:
		...
		with connection.cursor() as cursor:  
			sql = f"select ip_address,network_id from ipallocations where port_id = '{instance_id}'"
			cursor.execute(sql)
...
def router_info(gateway,network_id):
	try:
		passwd = subprocess.check_output(["grep","-i","neutron_db_pw","/capstone.d/password.txt"],universal_newlines=True).split('=')[1].replace("\n","")
		connection = pymysql.connect(host='localhost', \
									user='neutron', \
		...
		with connection.cursor() as cursor:  
			sql = f"SELECT routerports.router_id \
					...
					AND ipallocations.network_id = '{network_id}'"
			cursor.execute(sql)
			result = cursor.fetchall()
	finally:
		connection.close()
